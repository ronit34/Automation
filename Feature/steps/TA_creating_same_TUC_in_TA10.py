...
@then(u'Click on Add TUC in TA10')
def step_impl(context):
    button = context.driver.find_element(By.ID, "create_tuc")
    context.driver.execute_script("arguments[0].click();", button)


@then(u'Insert details in add TUC in TA10 for same name')
def step_impl(context):
...
@then(u'Click on create in add TUC in TA10')
def step_impl(context):
    button = context.driver.find_element(By.ID, "save_tuc")
    context.driver.execute_script("arguments[0].click();", button)


@then(u'Verify tenant create or not in TA10')
def step_impl(context):
    txt = "ICICI(2001)"
