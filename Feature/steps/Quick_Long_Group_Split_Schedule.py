...
new_quick_sms_input33Obj = CU.new_quick_sms_input33()

@then(u'select sender_id')
def step_impl(context):
    for element_id in new_quick_sms_input33Obj.new_quick_sms_input33_list.keys():
        context.driver.find_element(By.ID, element_id).send_keys(
            new_quick_sms_input33Obj.new_quick_sms_input33_list[element_id])
        print(f"{element_id}")

@then(u'click on split schedule to split')
def step_impl(context):
    time.sleep(2)
    ...
    context.driver.find_element_by_xpath(
        "/html/body/div[1]/div[2]/div[3]/div[2]/div[1]/div/div[7]/div[2]/div[2]/input").send_keys("11")
