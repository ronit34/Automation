...
new_unicode_sms_inputObj = CU.new_quick_sms_input2()

@then(u'insert data into Unicode sms without recipients')
def step_impl(context):
    for element_id in new_unicode_sms_inputObj.new_quick_sms_input2_list.keys():
        context.driver.find_element(By.ID, element_id).send_keys(
            new_unicode_sms_inputObj.new_quick_sms_input2_list[element_id])
        print(f"{element_id}")
