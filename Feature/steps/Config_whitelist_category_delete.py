...
@then(u'check delete icon mouse hover text - category_whitelist')
...
@then(u'CLick on delete of created whitelist category')
def step_impl(context):
    time.sleep(1)
    button = context.driver.find_element(By.XPATH, "//*[@id='delete']/img")
    context.driver.execute_script("arguments[0].click();", button)

@then(u'Check Message of delete pop up of whitelist category')
def step_impl(context):
    time.sleep(2)
    ...
    if text == "Name : SBI Cust":
        assert True, f"{text} is present"
    else:
        assert False, f"{text} is not present"


@then(u'Click on delete button to delete whitelist category')
def step_impl(context):
    time.sleep(4)
...
@then(u'Check selected whitelist category is deleted or not')
def step_impl(context):
    time.sleep(2)
    text = context.driver.find_element_by_xpath(
        "//*[@id='tab_content']/table/tbody/tr[2]/td[2]").text
    if text != "SBI Cust [1412]":
