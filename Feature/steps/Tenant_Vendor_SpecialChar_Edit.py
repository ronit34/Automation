...
@then(u'check edit button of Vendor is present')
def step_impl(context):
    time.sleep(1)
    context.driver.find_element_by_xpath(
        "//*[@id='pills-campaign']/div[2]/table/tbody/tr[2]/td[4]/a[1]/img").is_displayed()

@then(u'check edit icon mouse hover text - spl character')
def check_edit_button(context):
    time.sleep(2)
    action = ActionChains(context.driver);
    parent_level_menu = context.driver.find_element_by_xpath("//*[@id='pills-campaign']/div[2]/table/tbody/tr[3]/td[4]/a[1]")
    action.move_to_element(parent_level_menu).perform()

    hover_title = context.driver.find_element_by_xpath("//*[@id='pills-campaign']/div[2]/table/tbody/tr[3]/td[4]/a[1]").get_attribute('title')
    if hover_title == "Edit":
...
@then(u'click on edit button of Vendor')
def step_impl(context):
    time.sleep(1)
...
@then(u'Enter the details to edit into edit Vendor')
def step_impl(context):
    time.sleep(1)
    for element_id in option_vendor_editobj.Option_Vendor_edit_inputbox_list.keys():
        ...
        print(f"{element_id}")
    text = context.driver.find_element_by_id("vendor_telemar").get_attribute('value')
...
@then(u'click on update of edit Vendor')
def step_impl(context):
    time.sleep(5)
...
@then(u'verify the updated vendor is created or not')
def step_impl(context):
    time.sleep(1)
